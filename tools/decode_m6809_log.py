...
with open(r"..\sub_cpu_log","rb") as f:
    contents = f.read()
    dead_marker, = struct.unpack(">H",contents[-2:])

    contents = contents[:-8]
    if dead_marker != 0xDEAD:
        raise Exception("Corrupt sub CPU log, should end by 0xDEAD at offset -8")
...
# generated using log:     trace mame.tr,,,{tracelog "A=%02X, B=%02X, D=%04X, X=%04X, Y=%04X, U=%04X ",a,b,d,x,y,u}
lst = []
